...
use crate::codex_adapter::shim::{terminal_info, TerminalName};
...
pub(crate) fn set_default_colors_from_startup_probe(
    colors: Option<crate::codex_adapter::terminal_probe::DefaultColors>,
) {
...
mod imp {
    ...
    pub(super) fn requery_default_colors() {
        // cusa uses the bounded startup probe stub until OSC 10/11 query is ported.
    }

    fn query_default_colors() -> Option<DefaultColors> {
        crate::codex_adapter::terminal_probe::default_colors(
            crate::codex_adapter::terminal_probe::DEFAULT_TIMEOUT,
        )
        .ok()
        .flatten()
...
mod imp {
    ...
    fn query_default_colors() -> Option<DefaultColors> {
        crate::codex_adapter::terminal_probe::default_colors(crate::codex_adapter::terminal_probe::DEFAULT_TIMEOUT)
            .ok()
            .flatten()
            .map(|colors| DefaultColors {
                fg: colors.fg,
...
mod imp {
    ...
    #[cfg(any(unix, windows))]
    pub(super) fn set_default_colors_from_startup_probe(
        _colors: Option<crate::codex_adapter::terminal_probe::DefaultColors>,
    ) {
    }
